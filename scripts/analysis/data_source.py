...
LEADERS = {
    ...
    '光伏设备': [
        ...
        ('阳光电源', 'sz300274'), ('晶澳科技', 'sz002459'), ('天合光能', 'sh688599'),
    ],
    '医疗器械': [
        ('迈瑞医疗', 'sz300760'), ('联影医疗', 'sh688271'),
        ('鱼跃医疗', 'sz002223'), ('乐普医疗', 'sz300003'), ('新华医疗', 'sh600587'),
    ],
    # ---- 医药 ----
    '医药生物': [
    ...
    '半导体': [
        ('中芯国际', 'sh688981'), ('韦尔股份', 'sh603501'),
        ('北方华创', 'sz002371'), ('海光信息', 'sh688041'), ('长电科技', 'sh600584'),
    ],
    # ---- 汽车 ----
    '汽车': [
    ...
    '食品饮料': [
        ...
        ('伊利股份', 'sh600887'), ('海天味业', 'sh603288'), ('青岛啤酒', 'sh600600'),
    ],
    # ---- 周期 ----
    '房地产': [
        ('万科A', 'sz000002'), ('保利发展', 'sh600048'),
        ('招商蛇口', 'sz001979'), ('金地集团', 'sh600383'), ('华侨城A', 'sz000069'),
    ],
    '钢铁': [
        ('宝钢股份', 'sh600019'), ('中信特钢', 'sz000708'),
        ('华菱钢铁', 'sz000932'), ('鞍钢股份', 'sz000898'), ('河钢股份', 'sz000709'),
    ],
    '煤炭': [
        ('中国神华', 'sh601088'), ('陕西煤业', 'sh601225'),
        ('兖矿能源', 'sh600188'), ('中煤能源', 'sh601898'), ('潞安环能', 'sh601699'),
    ],
}
...
def get_board_leaders_fixed(sector_name: str) -> List[Dict]:
    ...
    查找顺序：
      1. 直接匹配板块名
      2. 通过 _SECTOR_PARENT 回退到父板块
      3. 默认回退到电力设备
    """
    # 优先直接匹配
    if sector_name in LEADERS:
        return LEADERS[sector_name]
    # 父板块回退
    parent = _SECTOR_PARENT.get(sector_name)
    if parent and parent in LEADERS:
        return LEADERS[parent]
    # 兜底
    return LEADERS.get('电力设备', [])
