...
def _fetch_constituent_data(stock_code: str) -> Optional[Dict]:
    ...
    # 近5日涨幅（新浪日K线）
    gain_5d = 0.0
    bars, bsrc, berr = get_daily_bars_sina(std_code, count=10)
    if bars and len(bars) >= 6:
        c0 = _safe_float(bars[0].get('close'))
        c5 = _safe_float(bars[-1].get('close'))
        if c0 > 0:
            gain_5d = (c5 - c0) / c0 * 100
...
def _fetch_board_data_dual(board: Dict) -> Dict:
    ...
    merged = {
        ...
        # 5日/20日: 仅A有
        'chg_5d': track_a.get('chg_5d') if track_a else None,
        'chg_20d': track_a.get('chg_20d') if track_a else None,
        # 成交额/换手率/量比: 仅A有
        'turnover': track_a.get('turnover') if track_a else None,
        'volume_ratio': track_a.get('volume_ratio') if track_a else None,
...
def check_sector_trend_v3(core_boards: List[Dict]) -> CheckResult:
    ...
    reason_parts.append(f"总分{total}分{'✅' if passed else '❌'}")
...
def check_sector_leaders_v3(core_boards: List[Dict], stock_code: str) -> CheckResult:
    ...
    reason = (f"板块活跃度{activity}/60({avg_5d:+.1f}%5日+{avg_today:+.1f}%今日+{limit_up}涨停) | "
